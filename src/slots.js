...
export function pickSlots(busy = [], count = 3, now = new Date()) {
  ...
  const free = candidateSlots(now).filter((when) => {
    const start = new Date(when.getTime() - cfg.booking.bufferMinutes * 60000);
    const end = new Date(
      when.getTime() + (mins + cfg.booking.bufferMinutes) * 60000,
    );
    return ![...busy, ...taken].some(
